...
export default function WalkingCaloriesCalculator() {
  ...
  useEffect(() => {
    const validateInput = () => {
      if (!weight || !duration || !walkType) return "";
      ...
      if (!MET_VALUES.hasOwnProperty(walkType)) return "Please select a valid walk type.";
      return "";
    };
  ...
  return (
    <div className="flex items-center justify-center h-screen bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 text-white">
      <div className="bg-gray-900 p-8 rounded-2xl shadow-2xl w-full max-w-lg border border-gray-700">
        <h2 className="text-3xl font-bold mb-6 text-center text-blue-400">Walking Calories Calculator</h2>
        <div className="flex flex-col gap-4">
          ...
          >
            <option value="" disabled>Select Walk Type</option>
            <option value="slow">Slow Walk</option>
            <option value="normal">Normal Walk</option>
            <option value="fast">Fast Walk</option>
            <option value="uphill">Uphill Walk</option>
            <option value="brisk">Brisk Walk</option>
          </select>
        </div>
